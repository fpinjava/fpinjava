...
public class SimpleRNG {
  ...
  // Failing 1
  public static Rand<Integer> nonNegativeLessThan_(int n) {
    return map(nonNegativeInt, x -> x % n);
  }

  // Failing 2
//public static Rand<Integer> nonNegativeLessThan__(int n) {
//  return  map(nonNegativeInt, i -> {
//    int mod = i % n;
//    return (i + (n - 1) - mod >= 0)
//        ? mod
//        : nonNegativeLessThan(n).apply(???);
//  }
//}

  // Failing 3
  public static Rand<Integer> nonNegativeLessThan___(int n) { 
    return rng -> {
      Tuple<Integer, RNG> t = nonNegativeInt(rng);
      int mod = t._1 % n;
      return (t._1 + (n - 1) - mod >= 0)
        ? new Tuple<>(mod, t._2)
        : nonNegativeLessThan(n).apply(rng);
    };
  }

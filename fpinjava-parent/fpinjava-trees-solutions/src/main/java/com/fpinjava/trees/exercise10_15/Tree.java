...
public abstract class Tree<A extends Comparable<A>> {
  ...
  protected abstract Tree<A> ins(A a);
  ...
  protected abstract Tree<A> rem(A a);

  public abstract boolean isEmpty();

  public abstract Tree<A> merge(Tree<A> a);

  protected abstract Tree<A> mrg(Tree<A> a);
  ...
  private static class T<A extends Comparable<A>> extends Tree<A> {
    ...
    public Tree<A> insert(A a) {
      Tree<A> t = ins(a);
      return t.height() > log2nlz(t.size()) * 100 ? balance(t) : t;
    }
    ...
    protected Tree<A> rem(A a) {
      if (a.compareTo(this.value) < 0) {
        return new T<>(left.rem(a), value, right);
      } else if (a.compareTo(this.value) > 0) {
        return new T<>(left, value, right.rem(a));
      } else {
    ...
    protected Tree<A> mrg(Tree<A> a) {
      ...
      if (a.value().compareTo(this.value) > 0) {
        return new T<>( left, value, right.mrg(new T<>(empty(), a.value(), a.right()))).mrg(a.left());
      }
      if (a.value().compareTo(this.value) < 0) {
        return new T<>( left.mrg(new T<>(a.left(), a.value(), empty())), value, right).mrg(a.right());
      }
      return new T<>(left.mrg(a.left()), value, right.mrg(a.right()));
    }
    ...
    public Tree<A> merge(A a, Tree<A> right) {
      return right.isEmpty()
          ? max().getOrThrow().compareTo(a) > 0
              ? insert(a)
              : new T<>(this, a, right)
          : max().flatMap(lmax -> right.min().map(rmin -> a.compareTo(lmax) > 0 && a.compareTo(rmin) < 0
              ? new T<>(this, a, right)
              : merge(right).insert(a))).getOrThrow();
    }

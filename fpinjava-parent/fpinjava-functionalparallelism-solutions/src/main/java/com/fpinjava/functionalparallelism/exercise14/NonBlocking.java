...
public class NonBlocking {
  ...
  public static <A> Par<A> choice(Par<Boolean> p, Par<A> t, Par<A> f) {
    return es -> (cb, ce) -> p.apply(es).apply(b -> {
      if (b) {
  ...
  public static <A> Par<A> choiceN(Par<Integer> n, List<Par<A>> ps) {
    return es -> (cb, ce) -> n.apply(es).apply(ind -> eval(es, () -> {
      try {
  ...
  public static <A, B> Par<B> flatMap(Par<A> p, Function<A, Par<B>> f) {
    return es -> (cb, ce) -> p.apply(es).apply(a -> f.apply(a).apply(es).apply(cb, ce), ce);
  }
  ...
  public static <A> Par<A> join(Par<Par<A>> p) {
    return es -> (cb, ce) -> p.apply(es).apply(p2 -> eval(es, () -> p2.apply(es).apply(cb, ce)), ce);
  }

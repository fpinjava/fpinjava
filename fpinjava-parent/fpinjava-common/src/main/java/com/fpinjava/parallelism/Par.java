package com.fpinjava.parallelism;
...
public interface Par<A> extends Function<ExecutorService, Future<A>> {
  ...
  static <A> Par<A> delay(Supplier<Par<A>> fa) {
  ...
  static <A> boolean equal(ExecutorService es, Par<A> p1, Par<A> p2) {
    try {
  ...
  static <A> Par<A> choice(Par<Boolean> cond, Par<A> t, Par<A> f) {
    return es -> {
      Boolean fu;
      try {
        fu = run(es, cond).get(); // Notice we are blocking on the result of `cond`.
      } catch (InterruptedException | ExecutionException e) {
        fu = false; // We must choose what to do in case of an exception
      }
      return fu
          ? t.apply(es)
          : f.apply(es);};
  ...
  static <A> Par<A> choiceN(Par<Integer> n, List<Par<A>> choices) {
    return es -> {
        int ind;
        try {
          ind = run(es, n).get();
        } catch (InterruptedException | ExecutionException e) {
          throw new IllegalStateException(e); // We can rethrow or choose a default index value
        }
        Par<A> a = choices.getAt(ind).get(); // Will throw an exception if index out of range.
        return run(es, a);
      };
  }

  static <A> Par<A> choiceViaChoiceN(Par<Boolean> a, Par<A> ifTrue, Par<A> ifFalse) {
    return choiceN(map(a, b -> b ? 0 : 1), List.list(ifTrue, ifFalse));
  }

  static <K, V> Par<V> choiceMap(Par<K> key, Map<K, Par<V>> choices) {
    return es -> {
        K k;
        try {
          k = run(es, key).get();
        ...
        return run(es, choices.get(k).getOrThrow());
      };
  }

  static <A, B> Par<B> chooser(Par<A> p, Function<A, Par<B>> choices) {
    return es -> {
        A a;
        try {
          a = run(es, p).get();
   ...
   */
  static <A, B> Par<B> flatMap(Par<A> p, Function<A, Par<B>> choices) {
    return es -> {
      A a;
      try {
        a = run(es, p).get();
  ...
  static <A> Par<A> choiceViaFlatMap(Par<Boolean> p, Par<A> t, Par<A> f) {
    return flatMap(p, b -> b ? t : f);
  }

  static <A> Par<A> choiceNViaFlatMap(Par<Integer> p, List<Par<A>> choices) {
    return flatMap(p, i -> choices.getAt(i).get()); // Throws an exception if index out of range
  }

  /*
   *  see nonblocking implementation in `NonBlocking.java`
   */
  static <A> Par<A> join(Par<Par<A>> a) {
    return es -> {
      Future<A> fpa;
      try {
        fpa = run(es, run(es, a).get());
      } catch (InterruptedException | ExecutionException e) {
        throw new IllegalStateException(e);
      }
      return fpa;
  ...
  static <A> Par<A> joinViaFlatMap(Par<Par<A>> a) {
    return flatMap(a, x -> x);
  }

  static <A, B> Par<B> flatMapViaJoin(Par<A> p, Function<A, Par<B>> f) {
    return join(map(p, f));
  }

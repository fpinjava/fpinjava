...
public class SimpleState {
  ...
  public static <S, A, B, C> State<S, C> map2_(State<S, A> ra, State<S, B> rb, Function<A, Function<B, C>> f) {
    return new State<>(s -> {
      Tuple<A, S> t1 = ra.run.apply(s);
      Tuple<B, S> t2 = rb.run.apply(t1._2);
  ...
  public static <S, A, B> State<S, B> flatMap(State<S, A> f, Function<A, State<S, B>> g) {
    return new State<>(s -> {
      Tuple<A, S> t = f.run.apply(s);
      return g.apply(t._1).run.apply(t._2); // We pass the new state along
      });
  }

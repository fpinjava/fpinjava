...
//        : f.apply(head()).apply(() -> foldRightHeapBased_(acc, stream.tail(), z, f));        
    ...
    public <U> U foldLeft__(Stream<T> stream, Supplier<U> z, Function<Supplier<U>, Function<T, U>> f) {
      return stream.isEmpty()
          ? z.get()
          : foldLeft__(stream.tail(), () -> f.apply(z).apply(stream.head()), f);
    }
    
    public <U> TailCall<U> foldLeft_(Stream<T> stream, Supplier<U> z, Function<Supplier<U>, Function<T, U>> f) {
      return stream.isEmpty()
          ? ret(z.get())

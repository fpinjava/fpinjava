...
public class Generator {
  ...
  public static State<RNG, Integer> limitedInteger = rng -> {
    Tuple<Integer, RNG> random = rng.nextInt();
    return new Tuple<>(Math.abs(random._1 % Math.max(1, rng.limit())), rng);
  };

  public static State<RNG, List<Integer>> integers = rng -> {
    Tuple<List<Tuple<Integer, RNG>>, RNG> result = List.range(0, rng.length()).foldLeft(new Tuple<>(List.list(), rng), tuple -> i -> {
      Tuple<Integer, RNG> t = integer(tuple._2);
      return new Tuple<>(tuple._1.cons(t), t._2);
    });
    List<Integer> list = result._1.map(x -> x._1);

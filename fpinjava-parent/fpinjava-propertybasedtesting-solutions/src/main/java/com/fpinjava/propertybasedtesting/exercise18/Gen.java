package com.fpinjava.propertybasedtesting.exercise18;
...
public class Gen<A> {
  ...
  public static Gen<Function<String, Integer>> genStringIntFn(Gen<Integer> g) {
    return g.map(i -> (s -> i));
  }

  public static <A> Gen<Function<String, A>> genStringFn(Gen<A> g) {
    Function<RNG, Tuple<Function<String, A>, RNG>> f = rng -> {
      Tuple<Integer, RNG> tuple = rng.nextInt();
      long seed = tuple._1;
      RNG rng2 = tuple._2;
      Function<String, A> h = s -> g.sample.run.apply(new SimpleRNG.Simple(seed ^ s.hashCode()))._1;
      return new Tuple<>(h, rng2);
    };
    State<RNG, Function<String, A>> s = new State<>(f);
    return new Gen<>(s);
  }

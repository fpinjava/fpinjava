...
/*
 * A slightly more complex version of stream. You may see the difference by running the cooresponding
...
public abstract class LazierStream<T> {
  ...
  private static <T> LazierStream<T> cons(Supplier<T> hd, Supplier<LazierStream<T>> tl) {
    return new Cons<T>(LazyVal.head(hd), tl);
  }
  ...
  public static <T> LazierStream<T> cons(List<T> list) {
    return list.isEmpty()
        ? empty()
        : new Cons<T>(LazyVal.head(() -> list.head(), list.head()), () -> cons(list.tail()));
  }

...
public class NonBlockingTest {
  ...
  public void testChoiceViaFlatMap() {
    ...
    NonBlocking.Par<List<Integer>> pl = choiceViaFlatMapViaJoin(pb, p1, p2);
  ...
  public void testChoiceNViaFlatMap1() {
    ...
    NonBlocking.Par<List<Integer>> pl = choiceNViaFlatMapViaJoin(pb, List.list(p1, p2, p3));
    List<Integer> result = NonBlocking.run(es,  pl).getOrThrow();
    assertEquals("[1, 4, 9, 16, 25, 36, 49, 64, 81, NIL]", result.toString());
  }
  ...
  public static <A> NonBlocking.Par<A> choiceViaFlatMapViaJoin(NonBlocking.Par<Boolean> p, NonBlocking.Par<A> t, NonBlocking.Par<A> f) {
    return NonBlocking.flatMapViaJoin(p, b -> b ? t : f);
  }

  public static <A> NonBlocking.Par<A> choiceNViaFlatMapViaJoin(NonBlocking.Par<Integer> p, List<NonBlocking.Par<A>> choices) {
    return NonBlocking.flatMapViaJoin(p, i -> choices.getAt(i).get());
  }

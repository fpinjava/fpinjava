...
public interface Par<A> extends Function<ExecutorService, Future<A>> {
  ...
  public static <A> Par<A> choice(Par<Boolean> cond, Par<A> t, Par<A> f) {
    return es -> {
      Boolean fu;
      try {
  ...
  public static <K, V> Par<V> choiceMap(Par<K> key, Map<K,Par<V>> choices) {
    return es -> {
        K k;
        try {
          k = run(es, key).get();
  ...
  public static <A, B> Par<B> chooser(Par<A> p, Function<A, Par<B>> choices) {
    return es -> {
        A a;
        try {
          a = run(es, p).get();
   ...
   */
  public static <A, B> Par<B> flatMap(Par<A> p, Function<A, Par<B>> choices) {
    return es -> {
      A a;
      try {
        a = run(es, p).get();
   ...
   */
  public static <A> Par<A> join(Par<Par<A>> a) {
    return es -> {
      Future<A> fpa;
      try {
  ...
  public static <A> Par<A> joinViaFlatMap(Par<Par<A>> a) {
    return flatMap(a, x -> x);
  }

  public static <A, B> Par<B> flatMapViaJoin(Par<A> p, Function<A, Par<B>> f) {
    return join(map(p, f));
  }

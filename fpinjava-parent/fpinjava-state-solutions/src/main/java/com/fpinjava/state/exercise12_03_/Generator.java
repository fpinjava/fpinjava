...
public class Generator {
  ...
  public static State<RNG, Integer> integer = RNG::nextInt;

  public static State<Tuple<RNG, Integer>, Integer> limitedInteger = tuple -> {
    Tuple<Integer, RNG> random = tuple._1.nextInt();
    return new Tuple<>(Math.abs(random._1 % tuple._2), new Tuple<>(random._2, tuple._2));
  };

  public static State<Tuple<RNG, Integer>, List<Integer>> integers = tuple -> {
    Tuple<List<Tuple<Integer, RNG>>, RNG> result = List.range(0, tuple._2).foldLeft(new Tuple<>(List.list(), tuple._1), t0 -> i -> {
      Tuple<Integer, RNG> t = t0._2.nextInt();
      return new Tuple<>(t0._1.cons(t), t._2);
    });
    List<Integer> list = result._1.map(x -> x._1);
    return new Tuple<>(list, new Tuple<>(result._2, 0));
  };
}
